                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCHILI ought to be enough for anybody." - Bill Gates, 2017
    ui->zCHILIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCHILISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCHILI->setEnabled(false);
        ui->pushButtonMintzCHILI->setToolTip(tr("zCHILI is currently disabled due to maintenance."));

        ui->pushButtonSpendzCHILI->setEnabled(false);
        ui->pushButtonSpendzCHILI->setToolTip(tr("zCHILI is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCHILIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCHILI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHILI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCHILI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCHILI in ") + 
...
void PrivacyDialog::on_pushButtonSpendzCHILI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCHILI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCHILI
        sendzCHILI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCHILI
    sendzCHILI();
}

void PrivacyDialog::on_pushButtonZCHILIControl_clicked()
{
    ZCHILIControlDialog* zCHILIControl = new ZCHILIControlDialog(this);
    zCHILIControl->setModel(walletModel);
    zCHILIControl->exec();
}

void PrivacyDialog::setZCHILIControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzCHILI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ChiliCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCHILIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCHILI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CHILI </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCHILI</b>";
    ...
    if (!ZCHILIControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCHILIControlDialog::GetSelectedMints();
    }
    ...
    ZCHILIControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CHILI, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCHILI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCHILI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCHILI "));
    ui->labelzCHILIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCHILISyncStatus->setVisible(fShow);
}
