...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XLR and zXLR money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XLR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XLR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XLR/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzxlr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXLR minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizechilicoinamount=<n>     " + strprintf(_("Keep N XLR anonymized (default: %u)"), 0) + "\n";
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZXLRMinted();
                        RecalculateZXLRSpent();
                    }
                    RecalculateXLRSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZXLRBackups = GetBoolArg("-backupzxlr", true);
        pwalletMain->setZXLRAutoBackups(fEnableZXLRBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
